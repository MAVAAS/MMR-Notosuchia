...
data<-read.table("dataMMRNotosuchia3Q.txt", header=T)
tree<-read.nexus("MMR5.treesQ.nex")
...
data<-read.table("dataMMRNotosuchia3Q.txt", header=T) # Reload data to plot it with log10 conversion
...
plot(NA,ylim=c(1L,length(tree2$tip.label)),xlim=lab,axes=FALSE,xlab="MMR (mLO2 h-1 g-0.85)")
lab <- lab[1L]:lab[2L]
