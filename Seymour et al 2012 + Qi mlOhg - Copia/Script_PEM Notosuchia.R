...
data<-read.table("dataMMRNotosuchia3.txt", header=T)
tree<-read.nexus("MMR5.trees.nex")
...
# Objects for retrodicted and fitted values
if(m == "none") {
...
data<-read.table("dataMMRNotosuchia3.txt", header=T) # Reload data to plot it with log10 conversion
...
plot(NA,ylim=c(1L,length(tree2$tip.label)),xlim=lab,axes=FALSE,xlab="MMR (mLO2 h-1 g-0.8536)")
lab <- lab[1L]:lab[2L]
...
# Compile retrodictions for extant taxa
